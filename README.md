...
│       ↓ collect_trade.py                                    │
│  trade_price 테이블                                         │
│  (보증금 / 월세 / 계약연월 — 대구 7개 구·군 × 6개월)      │
│                                                             │
│  [건물 유형·건축년도 기반 추정]                             │
│       ↓ official_price.py                                   │
│  official_price 테이블                                      │
│  (공시가격 추정값 — 다가구/오피스텔 유형별 계산)           │
└─────────────────────────────────────────────────────────────┘
...
│                                                             │
│  ① building_detail  →  건축 위험 점수 (최대 25점)          │
│     - 위반건축물이면 즉시 +20점                             │
│     - 건물 노후도 (30년 이상 +5점, 20년 이상 +3점 ...)     │
│     - 가구수/층수 비율 > 10 이면 +5점 (방 쪼개기 의심)     │
│                                                             │
│  ② trade_price      →  시세 이상 점수 (최대 25점)          │
│     - 최근 6개월 전세가율 = 평균보증금 ÷ 평균매매가        │
│     - 90% 이상 → +25점 / 80% → +20점 / 70% → +15점 ...    │
│                                                             │
│  ③ auction_history  →  임대인 위험 점수 (최대 30점)        │
│     - 경매 진행 중이면 +30점 (CODEF API 연동)              │
│     - 과거 경매 이력 있으면 +10점                          │
│                                                             │
│  ④ field_report     →  생활 안전 점수 + 현장 제보 패널티  │
│     - 기본 +5점 + 검증된 제보 임팩트 점수 누적 가산        │
│                                                             │
│  총점 (최대 100점) → risk_grade (SAFE / CAUTION / DANGER)  │
│                    → hri_score 테이블 저장                  │
└─────────────────────────────────────────────────────────────┘
                        ↓ REST API 응답
┌─────────────────────────────────────────────────────────────┐
...
│  - 건물별 HRI Score 리포트                                  │
│  - 경매 배당 보증금 회수율 시뮬레이션                      │
│  - 관심 건물 북마크 / 3개 건물 비교 대시보드               │
│  - 계약 만기 D-90 / D-30 / D-7 알림                        │
└─────────────────────────────────────────────────────────────┘
```
